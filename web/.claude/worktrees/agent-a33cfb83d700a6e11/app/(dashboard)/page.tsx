...
import { Eye, AtSign, PieChart, Smile, EyeOff, Trophy, ArrowRight } from "lucide-react";
import { loadFiltered } from "@/lib/page";
import { getHistory, getScore, hasData } from "@/lib/data";
import { opportunities, groupVisibility, gradeFor } from "@/lib/derive";
import { activeCount } from "@/lib/filters";
import {
  Card, Title, Section, RankBar, Badge, PageHeader, NoData, StatCard, MeterBar,
  StackBar, Legend, ProgressRow, GradeChip,
} from "@/components/ui";
import { Gauge, Donut, VBar, Trend } from "@/components/charts";
...
const CLASS_COLOR: Record<string, string> = { owned: "#10b981", earned: "#3b82f6", competitor: "#f43f5e", social: "#8b5cf6" };
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 stagger mt-2">
            <StatCard label="Visibility" brief={BRIEFS.visibility} icon={<Eye className={I} />} value={agg.visibility} decimals={1} unit="/100"
              delta={live ? d("visibility_score") : undefined} tone={agg.visibility >= 40 ? "good" : agg.visibility >= 15 ? "warn" : "bad"}
              footer={<MeterBar pct={agg.visibility} color={agg.visibility >= 40 ? "#22c55e" : agg.visibility >= 15 ? "#f59e0b" : "#ef4444"} />} />
            <StatCard label="Mention rate" brief={BRIEFS.mention_rate} icon={<AtSign className={I} />} value={agg.mention_rate} decimals={1} suffix="%"
              delta={live ? d("mention_rate") : undefined} tone={agg.mention_rate >= 25 ? "good" : agg.mention_rate >= 10 ? "warn" : "bad"}
              footer={<MeterBar pct={agg.mention_rate} color={agg.mention_rate >= 25 ? "#22c55e" : "#f59e0b"} />} />
            <StatCard label="Share of voice" brief={BRIEFS.sov} icon={<PieChart className={I} />} value={brandSov} decimals={1} suffix="%"
              delta={live ? d("brand_share_of_voice") : undefined}
              sub={`vs ${Math.max(0, agg.sov.length - 1)} tracked competitors`} />
            <StatCard label="Competitive rank" brief={BRIEFS.rank} icon={<Trophy className={I} />} value={brandRank > 0 ? brandRank : 0} prefix={brandRank > 0 ? "#" : ""}
              accent="#f59e0b" sub={`of ${agg.sov.length} brands in the set`} />
            <StatCard label="Positive answers" brief={BRIEFS.positive_answers} icon={<Smile className={I} />} value={agg.sent.positive || 0} accent="#22c55e"
              sub={`${posShare.toFixed(0)}% of ${agg.n} prompts`} />
            <StatCard label="Invisible on" brief={BRIEFS.invisible} icon={<EyeOff className={I} />} value={agg.n - agg.mentioned} accent="#ef4444" tone="bad"
              sub="prompts with no RISA mention" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-12 gap-5 mt-2">
            ...
            <Card className="col-span-12 lg:col-span-4 p-5"><Title brief="Visibility score broken down by buyer persona — where RISA wins or is absent.">By Persona</Title><VBar data={persona} height={210} /></Card>
            <Card className="col-span-12 lg:col-span-4 p-5"><Title brief="Visibility by query intent — discovery and comparison are the buying funnel.">By Intent</Title><VBar data={intent} height={210} /></Card>
          </div>
        </div>
              ...
              <div className="space-y-2">
                {opps.map((o, i) => (
                  <div key={i} className="flex items-start gap-3 border border-slate-200 rounded-lg p-3 ds-card-hover">
                    ...
                    {idxOf.has(o.prompt) && <Link href={`/prompts/${idxOf.get(o.prompt)}`} className="text-xs text-slate-600 hover:bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-lg shrink-0 transition">View</Link>}
                  </div>
                ))}
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-2.5">
                {(summary?.citations?.top_domains || []).slice(0, 5).map((dm) => (
                  <div key={dm.domain} className="flex items-center gap-2 text-sm">
                    <span className="w-2 h-2 rounded-sm shrink-0" style={{ background: CLASS_COLOR[dm.class] || "#cbd0e0" }} />
