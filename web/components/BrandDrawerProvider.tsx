...
type Row = { idx: number; prompt: string; persona: string; intent: string; position: number };
...
export function BrandDrawerProvider({ children }: { children: ReactNode }) {
  ...
  const noun = vm?.isBrand ? "RISA appears" : seg?.field === "name" ? `${seg.value} is mentioned` : `match "${title}"`;
                ...
                <div className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-50 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-slate-400 truncate mt-0.5">
                      {(p.persona || "").toUpperCase()}{p.intent ? " · " + p.intent : ""}{p.position > 0 ? ` · RISA #${p.position}` : ""}
                    </div>
                  </div>
